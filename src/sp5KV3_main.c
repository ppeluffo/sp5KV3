 ...
 *
 * Version 5KV3.
 * Esta es un reescritura de la versión anterior de modo de dejar un firmware mas limpio y eliminar
 * lo innecesario.
 * - Se reescribieron las librerias.
 * - Los semaforos de acceso a recursos son tipo mutex y el acceso se hace en modo persistente por un while
 * - Se utiliza FRTOS8.2 que nos provee nuevas facilidades como ser los timers y la comunicacion entre tareas
 * - Varias funciones ( watchdog, led, terminal ) se implementan en la funcion de callback de un timer lo que las
 *   hace mucho mas sencillas.
 * - TIMERS: se manejan en modo persistente con un while hasta que el valor de retorno sea TRUE.
 * - MSG INTERTASK: Se utiliza la modalidad de seteo de bits eSetBits para que no se traslapen los mensajes.
 * - BUG: Se agrega en tkCmd que se resetee el caracter de entrada c c/ciclo. El problema es que luego de un CR, si
 *        no se borra, queda en c hasta que se teclee otro, y esto hacia que no pudiera entrar en modo sleep.
 * - Consumos: La placa logica, sin el modem consume en modo sleepo 0.6mA. Estos son casi todos del standby de la
 *   fuente.
 * - Agrego 1 timer para salir del modo MONITOR o SERVICE y volver al modo normal
 * - Si estoy en modo MONITOR o SERVICE no apago la terminal.
 * - En wdg agrego un control para resetearme 1 vez al dia.
 * - En los estados de GPRS, agrego que si falla algo, espero 20mins antes de reiniciar los ciclos.
 *
 *
 *  NEW FEATURES:
 *  1) La terminal se apaga y entra en modo sleep a los 120s de inactividad del teclado.
 *  2) En caso de estar en modo service o monitor, no se apaga la terminal.
 *  3) El led solo se prende si esta la terminal activa.
 *  4) Sale solo a los 30mins del modo service o monitor.
 *  5) Los headers del frame indican el estado ( normal / service / monitor )
 *  5) Agrego un modo de debug "basico" de modo de poder prender o eliminar todos los mensajes.
 *
 *  -En MEM_drop hay que deshabilitar el watchdog porque sino se resetea por este ya que demora mucho
 *   la rutina.
 *
 *   SALIDAS:
 *   El dlg controla las 4 salidas de 2 formas: modo consigna / modo simple
 *   Modo simple: Las salidas se ponen a un valor dado ya sea por comandos ( valores almacenados en systemVars )
 *   o desde el servidor.
 *   Modo consigna: Se fijan la hora de la consigna A y la de la consigna B. En este modo se impide el seteo manual
 *   En c/frame se trasmite el valor de las salidas.
 *   Los comandos son:
 *   write param outputs modo { consigna | simple }
 *                       [A B C D] { 0 | 1 }
 *                       en
 *
 *  6) En default arranca en modo continuo para que lo antes posible se conecte.
 *     Agrego un comando que sea redial para que salga del modo en que esta y disque para reconfigurarse.
 *
 *	7) En tkGPRS pueden haber condiciones no consideradas que hagan que me quede en un determinado estado.
 *	   Para evitarlo, luego de c/transicion inicio un contador que al expirar va a hacer que se salga del
 *	   estado y se reinicie la maquina de estados.
 *
 * ----------------------------------------------------------------------------------------------------------------
 */
 ...
 *
 * INTERRUPCIONES:
 * Un problema que vemos es que el watchdog es activado por software y no fijo por fusible por el hecho que
 * hay veces que debemos deshabilitarlo ( service ).
 * Por otro lado, si configuramos PB2 para interrumpir por nivel, puede ocurrir que el MCP se bloquee y deje
 * el pin en 1 con lo que bloquea al micro y tambien al watchdog.
 *
 *
 *
 *
 ...
 * ******************************************************************************************************
 *
 * * P2: Pasa el +++AT al server ( aun no cayo el socket ). Buscar otra forma de cerrar el socket.
 *
 * Version 2.0.14: @2015-03-19
 *   LOG_LEVEL/DEBUG_LEVEL/SERVICEMODETIMER
 * - En tkDigital modifico los logs. El del DCD lo dejo solo con debugmode y el INT_FAIL en LOGINFO
 * - La salida del modo monitor es igual a la del modo service: por inactividad se resetea.
 * - Agrego un funcion que hace el reset del micro y mande la causa en los inits bajo el tag RC.
 * - Caudalimetro: Vemos que muchas veces de deja de contar. La H es que se bloquea el acceso al MCP por un
 *   tema de interrupciones.
 *   El nuevo esquema utiliza el MCP como un latch y el programa sale c/s a leer el MCP para ver si llego una
 *   interrupcion.
 *   FLIP-FLOP: Las placas que lo tienen, genera un flanco de bajada: 1->0.
 *   Las otras, generan un flanco de subida 0->1.
 *
 *   Deshabilito las interrupciones al micro por el caudalimetro.
 * - Implemento la configuracion del modo de gsm band.
 *
 *
 * Version 2.0.13: @2015-02-23
 *  - La nueva placa SP3CH tiene latches para los pulsos, por lo tanto, luego de detectarlos debo borrar su
 *    estado.
 *  - Los latches tienen D = 0, salida = Q, control = SD.
 *    Al arrancar SD = 0->1, queda en 1, salida Q = 1.
 *    Cuando llega un clock, Q = 0 ( porque D = 0 ).
 *    Para borrar el latch, ponemos SD = 0->1.
 *
 * Version 2.0.12: @2015-02-04
 *  - Corrijo que en el frame de init, el mmax se mandaba siempre 0. Ahora mando un float.
 *  - Arreglo que al medir los pulsos, pasaba un puntero pero luego los ponia en 0 y el valor siempre quedaba en 0.
 *  - Modifico el MCP_init() para que halla una funcion por c/MCP. Si el la tkDigital, el conteo de pulsos genera
 *    un INT1 FAIL, inicializo tambien el MCP23018.
 *  - Agrego SIGNAL_tkControlRestartModeServiceTimer() en tkControl. Si estoy en modo service y presiono una tecla
 *    reinicio el timer. Si no, cuando expira ( 15m ) se resetea.
 *  - En 8CH no debe mandar las consignas.
 *  - Arreglo que el modulo tkGprs trasmite mal el ancho de los pulsos.
 *
 *  Version 2.0.11: @2015-01-30
 *  - Queda solucionado la trasmision en float.
 *  - En la version de 8 canales (UTE), mido el ancho de los pulsos de los canales digitales
 *    en secs. Tambien modifico el formato de la trasmision para considerarlo
 *    Esto ya estaba en la version 1.9.2 pero lo habiamos removido.
 *  - Agrego un timer de 30 mins. para salir solo del modo service con reset del micro.
 *
 *  Version 2.0.10: @2014-12-23
 ...
 *  - Modifico que las magnitudes maximas/minimas sean float. ( sensores de 5mts p/Rivera )
 *    !! Falta que al trasmitir inicial envie los max. en float. Ojo al chequear contra la BD.
 *
 *
...
unsigned int i,j;
	...
	i2c_init();
	startToken = 0;
	//MCP_init();	Debo hacerlo luego de arrancar el MCP porque uso semaforo. Ver tkControl.

	/* Arranco el RTOS. */
	sem_SYSVars = xSemaphoreCreateMutex();
	semph_MEM = xSemaphoreCreateMutex();
	sem_TIMERS = xSemaphoreCreateMutex();

	/* Creo las tareas */
	xTaskCreate(tkTimers, "TIME", tkTimers_STACK_SIZE, NULL, tkTimers_TASK_PRIORITY,  &xHandle_tkTimers);
	...
	xTaskCreate(tkEventos, "EV", tkEventos_STACK_SIZE, NULL, tkEventos_TASK_PRIORITY,  &xHandle_tkEventos);
...
//		vCoRoutineSchedule();
		if ( terminal_isApagada() && gprsAllowSleep() ) {
	//	if ( ( systemVars.pwrMode == PWR_DISCRETO ) && terminal_isApagada() && gprsAllowSleep() ) {
			sleep_mode();
		}
	}
